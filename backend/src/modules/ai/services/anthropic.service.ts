 ...
 * Implements IAIService using Vercel AI SDK with Anthropic
...
export class AnthropicService implements IAIService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // Use configurable model, default to claude-3-5-sonnet
    this.modelName = this.configService.get<string>('AI_MODEL') || 'claude-3-5-sonnet-20241022';
    this.model = anthropic(this.modelName);

    this.logger.log(`Anthropic Service initialized with model: ${this.modelName}`);
  }
  ...
  async generateIcebreaker(context: GenerationContext): Promise<IcebreakerResult> {
    if (!this.isAvailable()) {
      throw new Error('AI service not available - ANTHROPIC_API_KEY not configured');
    }
